...
template <typename T> std::pair<T, T> range(int num_digits) {
  ...
  T last =
      num_digits < max_digits ? first * 10 - 1 : std::numeric_limits<T>::max();
...
void sprintf(benchmark::State &s) {
  ...
  while (s.KeepRunning()) {
    for (auto i : data) {
      ...
      result += std::sprintf(buffer, "%s", "HELLO");
    }
  }
...
void format_to(benchmark::State &s) {
  ...
  while (s.KeepRunning()) {
    for (auto i : data) {
      ...
      result += std::format_to(buffer, "{}", "HELLO") - buffer;
    }
  }
  benchmark::DoNotOptimize(result);
}
